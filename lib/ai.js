...
async function callAnthropic(messages, systemPrompt) {
  ...
  const response = await client.messages.create({
    model: 'claude-sonnet-4-6', // check for latest model name before using in a new project
...
async function callOpenAI(messages, systemPrompt) {
  ...
  const response = await client.chat.completions.create({
    model: 'gpt-4o', // check for latest model name before using in a new project
    max_tokens: DEFAULT_MAX_TOKENS,
...
async function callGemini(messages, systemPrompt) {
  ...
  const model = genAI.getGenerativeModel({
    model: 'gemini-1.5-pro', // check for latest model name before using in a new project
